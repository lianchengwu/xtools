...
**Confidence:** HIGH (crate versions and GUI APIs verified from crates.io + docs.rs). MEDIUM for compositor-dependent always-on-top / focus-steal on GNOME Wayland.

**Decision:** Use **one** GUI stack — **eframe 0.36.1 + egui 0.36.1** — for the launcher *and* the three tool windows. Do not mix toolkits. Shared look comes from a workspace `xtools-ui` crate (Visuals + widgets), not from three independently styled apps.
...
```bash
# Workspace root
cargo new --bin xtools-launcher
# then convert to a workspace; add sibling bins + libs

# Core GUI (all four binaries)
cargo add eframe@0.36.1
cargo add egui@0.36.1
cargo add egui_extras@0.36.1

# Timestamp binary
cargo add jiff@0.2.35
...
# Translate binary
cargo add ureq@3.4.0 --features json
...
Keep eframe **default features** (wayland + x11 + wgpu). Do not disable Wayland. Do not add `glow` unless wgpu transparency is black on this NVIDIA/compositor combo — then set `NativeOptions.renderer = Renderer::Glow` with the `glow` feature, do not enable both casually.
...
**Launcher (always-on-top orbital menu) — all in one process:**
...
**Tool windows (timestamp / json / translate) — one process each:**
...
**If session is wlroots Wayland (Sway, Hyprland, …):**
...
**If wgpu transparent window is a black rectangle on this NVIDIA laptop:**
...
---
*Stack research for: Rust Linux desktop orbital-launcher toolbox*
*Researched: 2026-08-19*
